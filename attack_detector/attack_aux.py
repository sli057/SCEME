...
def build_test_graph(net_name):
    if net_name == "VGGnet":
        ...
        raise ValueError("net {:s} does not implemented")
...
def build_physical_adv_graph(net_name='VGGnet_wt_context', H=None, W=None,
    ...
    with HiddenPrints():
        ...
        model_vars = set(tf.global_variables()) - sticker_vars  # only Faster-RCNN  variables
        ...
        # !!!! here optimizer not minimize loss, the gradient is to maximum loss!
        ...
        varops['smooth_loss'] = tf.reduce_sum(tf.image.total_variation(varops['noise'])) / varops[
            'mask_area'] / 1000  # use noise_inputs if do not blank sticker area at first
        if print_opt:
            varops['printab_pixel_element_diff'] = tf.squared_difference(varops['noise'] / 255.0, placeholders[
                'printable_colors'])  # change to [0,255] # use noise_inputs if do not blank sticker area at first
        ...
        op = tf.train.AdamOptimizer(learning_rate=lr, beta1=adam_beta1, beta2=adam_beta2,
                                    epsilon=adam_epslion).minimize(varops['adv_loss'],
                                                                   var_list=tf.get_collection('adv_var'))
